...
def get_kegg_files(wildcards, suffix):
    files = expand(
        config["rdir"] + "/kegg-reads-anvio/{smp}" + suffix,
...
rule mmseqs_profile_kegg_reads_anvio_summary:
    input:
        hits_tables=lambda wc: get_kegg_files(wc, suffix="_hits.txt"),
        hits_modules_tables=lambda wc: get_kegg_files(wc, suffix="_module_paths.txt"),
        modules_tables=lambda wc: get_kegg_files(wc, suffix="_modules.txt"),
    output:
        hits_summary=(config["rdir"] + "/kegg-reads-anvio/kegg-hits-summary.tsv.gz"),
    ...
    threads: 1
    log:
        config["rdir"] + "/logs/kegg-reads-anvio/kegg.summary.log",
    benchmark:
        config["rdir"] + "/benchmarks/kegg-reads-anvio/kegg.summary.bmk"
    message:
