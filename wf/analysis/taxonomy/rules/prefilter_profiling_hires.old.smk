rule prefilter_profiling_hires:
    input:
        ...
        + "/prefilter-profiling-coarse/{smp}.prefilter-coarse.ids",
    ...
    output:
        ...
        bac_ids=config["rdir"] + "/prefilter-profiling-hires/{smp}_Bacteria.txt",
        arc_ids=config["rdir"] + "/prefilter-profiling-hires/{smp}_Archaea.txt",
        vir_ids=config["rdir"] + "/prefilter-profiling-hires/{smp}_Viruses.txt",
    ...
    params:
        ...
        coarse_read_ids=(
            config["rdir"] + "/prefilter-profiling-coarse/{smp}.prefilter-coarse.ids"
        ),
    ...
    shell:
        ...
        {params.awk_bin} -vFS="\t" -vR={params.k_rtl} -vA={params.k_out_filt_tmp} -vB={output.read_ids} '{{if ($1 == "C" && $7 > R){{print $2"\t"$3 > A}}else{{print $2 > B}}}}' {params.k_out_filt} 

        # get tax domain
        {params.taxonkit_bin} lineage -j {threads} -i 2 --data-dir {params.prefilter_k2_db_hires_taxonomy} {params.k_out_filt_tmp} \
            | {params.awk_bin} -vFS="\t" -vL={params.label} '{{split($3, a, ";"); gsub("d__", "", a[1]); if(a[1]==""){{a[1]="Unclassified"}}; print $1 > L"_"a[1]".txt"}}'

        if [[ -s {params.label}_Bacteria.txt ]]; then
            cat {params.label}_Bacteria.txt >> {output.read_ids}
        else
            touch {params.label}_Bacteria.txt
        fi

        if [[ -s {params.label}_Archaea.txt ]]; then
            cat {params.label}_Archaea.txt >> {output.read_ids}
        else
            touch {params.label}_Archaea.txt
        fi

        if [[ -s {params.label}_root.txt ]]; then
            cat {params.label}_root.txt >> {output.read_ids}
        else
            touch {params.label}_root.txt
        fi

        if [[ ! -s {params.label}_Viruses.txt ]]; then
            touch {params.label}_Viruses.txt
        fi
        ...
        {params.taxonkit_bin} lineage -j {threads} -i 2 --data-dir {params.prefilter_k2_db_hires_taxonomy} {params.k_out_filt_tmp} \
            | {params.awk_bin} -vFS="\t" -vL={params.label} '{{split($3, a, ";"); gsub("d__", "", a[1]); if(a[1]==""){{a[1]="Unclassified"}};b[a[1]]++}}END{{for (i in b){{print L"\t"i"\t"b[i]}}}}' \
            | {params.csvtk_bin} add-header -t -n 'label,domain,counts' | gzip > {output.k_hires_out_tax}



        pigz -p {threads} {params.k_out} {params.k_report_filt} {params.k_out_min}
