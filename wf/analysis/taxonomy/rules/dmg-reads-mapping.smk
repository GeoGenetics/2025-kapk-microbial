rule reads_dmg_mapping:
    ...
    params:
        ...
        + "/{dr_fraction}"
        + "/{smp}.dmg-reads.fq.gz",
        bowtie2_db=config["bowtie2_tp_db"],
        ...
        bm_mapq=config.get("bam_mapq", 20),
    log:
        config["rdir"]
        + "/logs/reads-dmg-mapping/"
        + "{indata}"
        + "/k{tp_bowtie2_k}"
        + "/{read_ani}"
        + "/weight-{weight}"
    ...
    shell:
        ...
        ps -Af | grep bowtie2 | grep "{params.bowtie2_db}" > /dev/null
        if [ $? -eq 0 ]; then
        ...
        if [ ${{IS_FASTQ}} = "FASTQ" ]; then
            # Align reads
            {params.bowtie2_bin} -x {params.bowtie2_db} \
                -t \
                -p {threads} \
                -q -U {input.reads_dmg} \
        ...
        {params.samtools_bin} view -bSq {params.bm_mapq}  {params.bowtie2_sam_tmp} \
            | {params.samtools_bin} sort -@ {threads} -T {params.bowtie2_tmp_dir} {params.samtools_sort_parms} -O BAM -o {params.bowtie2_bam}
