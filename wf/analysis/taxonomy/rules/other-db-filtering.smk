def get_other_len(db):
    for entry in config["db_other"]:
        for key, value in entry.items():
            if key == db:
                return value["db_genome_sizes"]


rule other_db_filtering:
    ...
    params:
        ...
        bowtie2_bam_dedup=config["rdir"]
        + "/other-filtering/{indata}/k{tp_bowtie2_k}/{read_ani}/{other_db}/{smp}.other.dedup.bam",
        ...
        tp_sizes=lambda wc: get_other_len(db=wc.other_db),
        tp_norm_scale=config["tp_norm_scale"],
        tp_extra=config["tp_extra"],
        profiling_data_input="{indata}",
        rdir=config["rdir"]
        + "/other-filtering/{indata}/k{tp_bowtie2_k}/{read_ani}/{other_db}",
        wdir=config["wdir"],
    log:
        config["rdir"]
        + "/logs/other-filtering/{indata}/k{tp_bowtie2_k}/{read_ani}/{other_db}/{smp}.other-filtering.log",
    benchmark:
        (
            config["rdir"]
            + "/benchmarks/other-filtering/{indata}/k{tp_bowtie2_k}/{read_ani}/{other_db}/{smp}.other-filtering.bmk"
        )
    conda:
