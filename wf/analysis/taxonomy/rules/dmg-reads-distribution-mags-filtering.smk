def get_dr_len(db):
    for entry in config["db_mag_distribution"]:
        for key, value in entry.items():
            if key == db:
                return value["db_mag_sizes"]


rule reads_dmg_distribution_filtering:
    ...
    params:
        ...
        bowtie2_bam_dedup=config["rdir"]
        + "/reads-dmg-distribution-mags-filtering/{indata}/k{tp_bowtie2_k}/{read_ani}/weight-{weight}/{mag_db}/{dr_fraction}/{smp}.dmg-reads.dedup.bam",
        ...
        tp_sizes=lambda wc: get_dr_len(db=wc.mag_db),
        tp_norm_scale=config["tp_norm_scale"],
        tp_extra=config["tp_extra"],
        profiling_data_input="{indata}",
        rdir=config["rdir"]
        + "/reads-dmg-distribution-mags-filtering/{indata}/k{tp_bowtie2_k}/{read_ani}/weight-{weight}/{mag_db}/{dr_fraction}",
        wdir=config["wdir"],
    log:
        config["rdir"]
        + "/logs/reads-dmg-distribution-mags-filtering/{indata}/k{tp_bowtie2_k}/{read_ani}/weight-{weight}/{mag_db}/{dr_fraction}/{smp}.dmg-reads.taxonomic-profiling.log",
    benchmark:
        (
            config["rdir"]
            + "/benchmarks/reads-dmg-distribution-mags-filtering/{indata}/k{tp_bowtie2_k}/{read_ani}/weight-{weight}/{mag_db}/{dr_fraction}/{smp}.dmg-reads.taxonomic-profiling.bmk"
        )
    conda:
