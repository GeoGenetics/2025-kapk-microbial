...
def get_dmg_files_bm(wildcards, weight, indata, k, ani, dr_fraction, bm_read_ani):
    files = expand(
        config["rdir"]
        + "/reads-dmg-metaDMG/"
        + indata
...
rule reads_dmg_metaDMG_summary:
    wildcard_constraints:
        indata="prefilter|standard|raw",
        weight="0|1",
        dr_fraction="damaged|non-damaged",
    input:
        dmg_tables=lambda wc: get_dmg_files_bm(
            wc,
            weight=wc.weight,
            indata=wc.indata,
            k=wc.tp_bowtie2_k,
