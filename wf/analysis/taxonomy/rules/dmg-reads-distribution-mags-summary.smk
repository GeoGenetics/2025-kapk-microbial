...
def get_tp_mapping_inputs_dr(wildcards, indata, k, ani, weight, mag_db, dr_fraction):
    files = expand(
        config["rdir"]
...
def get_tp_stats_files_dr(wildcards, indata, k, ani, weight, mag_db, dr_fraction):
    files = expand(
        config["rdir"]
...
def get_tp_stats_filtered_files_dr(
    wildcards, indata, k, ani, weight, mag_db, dr_fraction
):
    files = expand(
        config["rdir"]
        + "/reads-dmg-distribution-mags-filtering/"
...
rule reads_dmg_distribution_summary:
    wildcard_constraints:
        indata="prefilter|standard|raw",
        weight="0|1",
        dr_fraction="damaged|non-damaged",
    input:
        tp_map_stats=lambda wc: get_tp_mapping_inputs_dr(
            wc,
            indata=wc.indata,
            k=wc.tp_bowtie2_k,
            ani=wc.read_ani,
        ...
        tp_stats=lambda wc: get_tp_stats_files_dr(
            wc,
            indata=wc.indata,
            k=wc.tp_bowtie2_k,
            ani=wc.read_ani,
        ...
        tp_stats_filtered=lambda wc: get_tp_stats_filtered_files_dr(
            wc,
            indata=wc.indata,
            k=wc.tp_bowtie2_k,
            ani=wc.read_ani,
    ...
    log:
        config["rdir"]
        + "/logs/reads-dmg-distribution-mags-summary/"
        + "{indata}"
        + "/k{tp_bowtie2_k}"
        + "/{read_ani}"
        + "/weight-{weight}"
    ...
    benchmark:
        (
            config["rdir"]
            + "/benchmarks/reads-dmg-distribution-mags-summary/"
            + "{indata}"
            + "/k{tp_bowtie2_k}"
            + "/{read_ani}"
            + "/weight-{weight}"
